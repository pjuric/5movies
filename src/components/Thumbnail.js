...
function Thumbnail({ movie }) {
    ...
    const id = movie.id
    return (
        <div className="transition duration-200 ease-in transform hover:scale-95 hover:z-50 space-y-1 md:space-y-3 oveflow-hidden relative text-white-primary rounded-md cursor-pointer bg-black-full h-20 sm:h-28 md:h-40  w-full lg:h-52 xl:h-64 xl:w-full flex flex-col justify-center items-center">
            <Link to={`/movie/${id}`}>
            <img 
                alt=""
                src={
